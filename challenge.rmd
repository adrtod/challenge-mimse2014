...
| <i class="fa fa-cloud-upload fa-2x" aria-hidden="true"></i> \ **`r n_submissions`** | <i class="fa fa-users fa-2x" aria-hidden="true"></i> \ **`r n_team`** | <i class="fa fa-calendar fa-2x" aria-hidden="true"></i> \ **`r countdown(deadline)`** |
|:-------------:|:-------------:|:-------------:|
...
Le 4 janv. 2015 :
  ~ Note sur la prise en compte des coûts : [<i class="fa fa-download fa-2x" aria-hidden="true"></i>](note_challenge.pdf)
  
Le 3 nov. 2014 :
  ~ Les scores sont maintenant calculés sur l'ensemble du jeu test et non plus sur un sous ensemble. Le [classement][Classement] et l'[historique][Historique des soumissions] ont été mis à jour en conséquence. La date limite des contributions a été repoussée au `r format(deadline, "%A %d %b %Y")`.
...
1. Envoyer un email à <`r email`> contenant les informations suivantes :
    ...
    - au moins une adresse email associée à un compte Dropbox [<i class="fa fa-dropbox fa-2x" aria-hidden="true"></i>](https://www.dropbox.com/)
...
| ---- | ---- | ----------- | ----- |
| Apprentissage | `data_train.rda` | `data.frame` avec ```r nrow(data_train)``` lignes/clients et ```r ncol(data_train)``` colonnes/variables | [<i class="fa fa-download fa-2x" aria-hidden="true"></i>](data/data_train.rda) |
| Test | `data_test.rda` | `data.frame` avec ```r nrow(data_test)``` lignes/clients et ```r ncol(data_test)``` colonnes/variables | [<i class="fa fa-download fa-2x" aria-hidden="true"></i>](data/data_test.rda) |
